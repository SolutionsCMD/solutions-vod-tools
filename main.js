...
const { app, BrowserWindow, Tray, Menu, shell, dialog, ipcMain, session } = require('electron');
...
// Cookies for yt-dlp's --cookies flag. Written by the in-app YouTube sign-in
// flow (Settings tab). Format is Netscape cookies.txt; yt-dlp reads it
// directly, bypassing the DPAPI issues with Chrome/Edge cookie databases.
const cookiesFile = path.join(userData, 'cookies.txt');
const YT_COOKIE_PARTITION = 'persist:yt-signin';
...
function isYoutubeCookieDomain(domain) {
  // YouTube auth spans .google.com (consent + auth) and .youtube.com.
  // Save both so yt-dlp has everything it needs after a fresh sign-in.
  const d = (domain || '').toLowerCase();
  return d.endsWith('youtube.com') || d.endsWith('google.com') || d.endsWith('googlevideo.com');
}

function cookieToNetscapeLine(c) {
  // Electron Cookie shape: { name, value, domain, hostOnly, path, secure,
  //   httpOnly, session, expirationDate, sameSite }
  let domain = c.domain || '';
  // Netscape format: leading-dot domain means "include subdomains."
  // hostOnly cookies bind to a specific host only.
  ...
  // Session cookies (no expirationDate) → expiration 0 in cookies.txt; yt-dlp
  // treats those as session cookies (still sent with requests).
  const expiration = c.expirationDate ? Math.floor(c.expirationDate) : 0;
  const name = c.name || '';
  const value = c.value || '';
  return [domain, includeSubdomains, cookiePath, secure, expiration, name, value].join('\t');
}

async function writeCookiesFile() {
  try {
    const ses = session.fromPartition(YT_COOKIE_PARTITION);
    const cookies = await ses.cookies.get({});
    const filtered = cookies.filter(c => isYoutubeCookieDomain(c.domain));
    const header = [
      '# Netscape HTTP Cookie File',
      '# Generated by Solutions VOD Tools — YouTube sign-in',
      `# ${new Date().toISOString()}  ${filtered.length} cookies`,
      '',
    ];
    const body = filtered.map(cookieToNetscapeLine);
    await fs.promises.writeFile(cookiesFile, header.concat(body).join('\n'), 'utf8');
    logInfo(`[yt-signin] wrote ${filtered.length} cookies to ${cookiesFile}`);
    return { ok: true, count: filtered.length, path: cookiesFile };
  } catch (err) {
    logError(`[yt-signin] failed to write cookies: ${err.message}`);
    return { ok: false, error: err.message };
  }
}

ipcMain.handle('cookies:signin', async () => {
  ...
  return new Promise((resolve) => {
    const win = new BrowserWindow({
      ...
      autoHideMenuBar: true,
      webPreferences: {
        partition: YT_COOKIE_PARTITION,
        contextIsolation: true,
        nodeIntegration: false,
        // Use a stock Chrome UA — YouTube's auth flow rejects/degrades when it
        // sees "Electron" in the UA string.
        // (We override on the webContents below; this just sets the default.)
      },
    });
    youtubeSignInWindow = win;
    win.webContents.setUserAgent(CHROME_UA);

    // Block any new windows the page might try to open (e.g., terms popups);
    ...
    win.loadURL('https://www.youtube.com/').catch(err => {
      logError(`[yt-signin] loadURL failed: ${err.message}`);
    });
    ...
    win.on('closed', async () => {
      ...
      if (resolved) return;
      resolved = true;
      const result = await writeCookiesFile();
      resolve(result);
    });
  });
});
